...
function [U,u] = EuropeanLocVolLSML1D(S,K,T,r,sig,payoff,phi,epf,epc,xf,xc,xls,M,op,limited)
%
...
% down x and U so that K=1.
  ...
  for step = 1:M
    ...
    %
    Sigma = spdiags(sig(xsig*K0,max(0,T-tn)),0,Nls,Nls);
    if (Nf > 0)
      Lf = Sigma.^2*Lxxf + Lx0f;
      Cf = If0-beta0*Lf;
      [Qf,Rf]=qr(Cf(:,2:Nf-1));
      Cf = Cf(:,[1 Nf]);
    end
    Lc = Sigma.^2*Lxxc + Lx0c;
    Cc = Ic0-beta0*Lc;
    [Qc,Rc]=qr(Cc(:,2:Nc-1));
    Cc = Cc(:,[1 Nc]);  
    %
