...
function [U]=EuLocVolRBFPU1D(S,K,T,r,sig,payoff,phi,ep,xc,M,np,op)
% Compute the value of a European option with local vol using RBF-PUM approximation
...
Lxx = spalloc(N,N,sum(Ni.^2));
Lx0 = spalloc(N,N,sum(Ni.^2));
for i = 1:np
    ...
    Lxx(indlocpts{i},indlocpts{i}) = Lxx(indlocpts{i},indlocpts{i}) + Bxx;
    Lx0(indlocpts{i},indlocpts{i}) = Lx0(indlocpts{i},indlocpts{i}) + Bx0;
...
% Time-stepping loop
for n = 1:M
    Sigma = spdiags(sig(r,S,xsig,max(0,T-tn)),0,N,N); 
    L = Sigma^2*Lxx + Lx0;
...
for i=eparts
  U(Elocpts{i}) = U(Elocpts{i}) + E{i}*u(indlocpts{i}); 
end 
