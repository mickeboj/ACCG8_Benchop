...
function [U]=AmRBFPU1D(S,K,T,r,sig,payoff,phi,ep,xc,M,np,op,epen)
% Compute the value of an American option using uniform RBF-PUM approximation
% and penalty approach
...
% epen size of the penalty parameter
%
...
%   sw(indlocpts{i}) =   sw(indlocpts{i}) + Wxx{i};
end
...
% Basis of payoff, needed for penalty
q = K-xc;

% Initial condition. Note that boundaries are at the next time
u0 = payoff(xc,K,r,0);
P = (epen*r*K)./(u0+epen-q); % Penalty
rhs(:,1) = u0 + beta0*P; 
tn = k(1);
...
for n = 1:M
    ...
    P = (epen*r*K)./(u+epen-q); % Divided by K above and below
