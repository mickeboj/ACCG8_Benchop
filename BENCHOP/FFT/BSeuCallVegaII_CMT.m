...
function [U,errRel]=BSeuCallVegaII_CMT(S,K,T,r,sigma)
S=S(:)';
...
CC=T*par(1)*exp(1i*b*z).*exp(Kf(z-1i*(alpha+1),T,r,0,par)-r*T);
...
% [~,~,~,V0]=BSCall_exact(S0,K0,T,r,par);
% errRel=(P0-V0)./V0

% % Plot solution
% SGrid=60:1:160;
% [~,~,~,VegaGrid]=BSCall_exact(SGrid,K0,T,r,par);
% plot(SGrid,F(SGrid),SGrid,VegaGrid,'--');
