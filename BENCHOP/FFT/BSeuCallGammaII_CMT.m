...
function [U,errRel]=BSeuCallGammaII_CMT(S,K,T,r,sigma)
S=S(:)';
...
CC=exp(1i*b*z).*exp(Kf(z-1i*(alpha+1),T,r,0,par)-r*T);
P=dz*exp((alpha-1)*x-log(K0)).*real(fft(CC.*w))/pi;
F=griddedInterpolant(fliplr(S),fliplr(P),'spline');
P0=F(S0);
% toc

% Compute relative error
% [~,~,G]=BSCall_exact(S0,K0,T,r,par);
% errRel=(P0-G)./G

% % Plot solution
% SGrid=60:1:160;
% [~,~,GammaGrid]=BSCall_exact(SGrid,K0,T,r,par);
% plot(SGrid,F(SGrid),SGrid,GammaGrid,'--');
