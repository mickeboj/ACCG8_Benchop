...
function [U]=BSeuCallVegaII_FGL(S,K,T,r,sigma)
% function [U]=BSeuCallVegaII_FGL(S,K,T,r,sigma);
%
% ----------input---------------------------
%
...
% ----------output---------------------------
%
%   U: Black Scholes European call option Vega
%
% (C) 2014 Magnus Wiktorsson
S=S(:)';
IS=ones(1,length(S));
load glax50
U=fouriervega(@BSKf,IS,IS,log(K)*IS,log(S),r*IS,T*IS,sigma*IS,x,we);

function [Vega,ak]=fouriervega(Kf,C,p,k,s0,r,Tau,param,x,we)
%function [Vega,ak]=fouriervega(Kf,C,p,k,s0,r,delta,Tau,param,x,we)
% calculates vega using inverse 
...
%
%   ----------------output----------------------------
%    Vega: calculated vega of option
...
Ik=ones(1,length(k));
Ix=ones(length(x),1);
anint=[max(0.999*max(anintpf(:,1)+sign(anintpf(:,1))*0.01,anintcf(1)-p'),-1e4),min(0.999*min(anintpf(:,2)+sign(anintpf(:,1))*0.01,anintcf(2)-p'),1e4)];
ak=goldsearch_spec(Kf,pf,anint,5e-1,k,Tau,r(:)',s0(:)',param,C,p); 
%mean(ak)
%ak=-2200*Ik;
z=x*Ik-1i*Ix*ak;
Vega = Tau*param(1).*exp(-r.*Tau).*(we'*real(exp(Kf(z-1i*Ix*p,Ix*Tau,Ix*r,Ix*s0,param)+pf(z,Ix*C,Ix*p)+(-1i*z.*(Ix*k)))))/pi;
