...
function [U]=BSeuCallDeltaI_FGL(S,K,T,r,sigma)
% function [U]=BSeuCallDeltaI_FGL(S,K,T,r,sigma);
%
% ----------input---------------------------
%
...
% ----------output---------------------------
%
%   U: Black Scholes European call option Delta
%
...
IS=ones(1,length(S));
load glax50
U=fourierdelta(@BSKf,IS,IS,log(K)*IS,log(S),r*IS,T*IS,sigma*IS,x,we);


function [U]=fourierdelta(Kf,C,p,k,s0,r,Tau,param,x,we)
%function [HedgingW,aK]=fourierdelta(Kf,C,p,k,s0,r,delta,Tau,param,x,we)
% calculates the delta hedge using inverse 
...
pf=inline('log(sign(2*C-1).*p)+betac(p,sign(2*C-1).*(i*x)+(p-1).*(C-1),0)','x','C','p');
if all(p==1)
...
%ak=goldsearch_spec(Kf,pf,anint,1e-5,k,Tau,r(:)',s0(:)',param,C,p);
%mean(ak)
ak=6.96*Ik;
z=x*Ik-1i*Ix*ak;
U = exp(-r.*Tau).*(we'*real(exp(Kf(z-1i*Ix*p,Ix*Tau,Ix*r,Ix*s0,param)-Ix*s0+pf(z,Ix*C,Ix*p)+(-1i*z.*(Ix*k)))))/pi;
...
m=a+gr*(b-a);
l=a+(1-gr)*(b-a);
fm=(Kf(-1i*(m+p),T,r,s0,par)-s0+pf(-1i*m,C,p)+(-m.*k));
fl=(Kf(-1i*(l+p),T,r,s0,par)-s0+pf(-1i*l,C,p)+(-l.*k));
...
for i=1:n
 ...
 m(I)=a(I)+gr*(b(I)-a(I));
 fl(I)=fm(I);
 fm(I)=(Kf(-1i*(m(I)+p(I)),T(I),r(I),s0(I),par)-s0(I)+pf(-1i*m(I),C(I),p(I))+(-m(I).*k(I)));
 end
 I=~I;
 if any(I)
 b(I)=m(I);
 ...
 fm(I)=fl(I);
 fl(I)=(Kf(-1i*(l(I)+p(I)),T(I),r(I),s0(I),par)-s0(I)+pf(-1i*l(I),C(I),p(I))+(-l(I).*k(I)));
 end
 end
a=(a+b)/2;
