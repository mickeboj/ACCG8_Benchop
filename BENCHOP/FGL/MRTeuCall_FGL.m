...
function [U]=MRTeuCall_FGL(S,K,T,r,sig,lambda,gamma,delta)
% function [U]=MertonCall_FGL(S,K,T,r,sig,lambda,gamma,delta);
%
% ----------input---------------------------
%
...
% ---------------output-------------------
% 
%      U: Merton European Call option price 
%
% (C) 2014 Magnus Wiktorsson
S=S(:)';
IS=ones(1,length(S));
load glax50
U=fourieropt(@MertonKf,IS,IS,log(K)*IS,log(S),r*IS,T*IS,[sig,lambda,gamma,delta],x,we);
