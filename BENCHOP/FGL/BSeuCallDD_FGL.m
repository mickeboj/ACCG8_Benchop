...
function [U]=BSeuCallDD_FGL(S,K,T,r,sigma,D,alpha)
% function [U]=BSeuCall_FGL(S,K,T,r,sigma,D,alpha);
%
% ----------input---------------------------
%
...
%   sigma: BM-vol
%       D: relative dividend
%   alpha: relative time to dividend as fraction of T 
%
%   
%  Note that for S,K,T and r that they should either be 1-dim
%  or have matching dimensions
%
%  If alpha>1 then the price collapses to the standard Black Scholes price
%
% ----------output---------------------------
%
%   U: Black Scholes discrete dividend European call option price
%
...
IS=ones(1,length(S));
load glax50
U=fourieropt(@BSddKf,IS,IS,log(K)*IS,log(S),r*IS,T*IS,[sigma D alpha*T],x,we);
...
%ak=goldsearch_spec(Kf,pf,anint,1e-8,k,Tau,r(:)',s0(:)',param,C,p);
%mean(ak)
ak=15.6*Ik;
z=x*Ik-1i*Ix*ak;
