...
function [U]=BSeuCallDeltaII_FGL(S,K,T,r,sigma)
% function [U]=BSeuCallDeltaII_FGL(S,K,T,r,sigma);
%
% ----------input---------------------------
%
...
function [HedgingW,ak]=fourierdelta(Kf,C,p,k,s0,r,Tau,param,x,we)
...
%
%   ----------------output----------------------------
%    HedgingW : calculated hedge weight
...
z=x*Ik-1i*Ix*ak;
HedgingW = min(exp(-r.*Tau).*(we'*real(exp(Kf(z-1i*Ix*p,Ix*Tau,Ix*r,Ix*s0,param)-Ix*s0+pf(z,Ix*C,Ix*p)+(-1i*z.*(Ix*k)))))/pi,1);
